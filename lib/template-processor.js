...
`;
  ...
  generateToolFiles(outputDir, { operations, authInfo, authEnvVar }) {
    ...
    const toolsIndex = `${operations.map(op => 
      `import ${op.operationId} from './${op.operationId}.js';`
    ).join('\n')}
...
import { formatResponse } from "../utils/response.js";${hasAuth ? `
...
export default {
  ...
  schema: {
    ...
    description: "${operation.description || operation.summary}",${hasParams || hasBody ? `
...
${hasParams ? `
      const { ${operation.parameters.map(p => p.name).join(', ')} } = input;` : ''}${hasBody ? `
      ...
      //   method: '${operation.method}',${hasAuth ? `
      //   headers: {
      ...
      return formatResponse({
        ...
        data: {
          ...
          path: "${operation.path}",${hasParams ? `
          parameters: { ${operation.parameters.map(p => p.name).join(', ')} },` : ''}${hasBody ? `
          body: requestBody,` : ''}
...
`;
    ...
    writeFileSync(join(outputDir, 'src', 'tools', `${operation.operationId}.js`), toolContent);
  }
