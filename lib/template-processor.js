...
export class TemplateProcessor {
  async generateFiles(outputDir, context) {
    ...
    this.generateCliArgs(outputDir, { authInfo });
...
// Make CLI options available globally
...
`;
    ...
    if (authInfo.hasAuth) {
      if (authInfo.type === "bearer") {
        authJs = `import { getAuthToken } from "./cli-args.js";
...
`;
        ...
        authJs = `import { getApiKey } from "./cli-args.js";
...
`;
        ...
        authJs = `import { getBasicAuth } from "./cli-args.js";

export function getAuthConfig() {
  const { username, password } = getBasicAuth();
...
`;
  ...
  generateCliArgs(outputDir, { authInfo }) {
    let cliArgsJs = "";
    if (authInfo.hasAuth) {
      if (authInfo.type === "bearer") {
        cliArgsJs = `import { program } from "commander";
...
`;
    ...
    writeFileSync(join(outputDir, "src", "utils", "cli-args.js"), cliArgsJs);
  }
...
${
  hasParams
    ...
    : ""
      ...
      let url = \`\${global.program.baseUrl || '${baseUrl}'}${operation.path}\`;
      ${
        pathParams.length > 0
          ? `
...
`;
  ...
  generateReadme(
    ...
    if (authInfo.hasAuth) {
      if (authInfo.type === "bearer") {
        authExample = `npm start -- --token your_token_here --base-url ${baseUrl}`;
      } else if (authInfo.type === "apiKey") {
        authExample = `npm start -- --api-key your_api_key_here --base-url ${baseUrl}`;
      } else if (authInfo.type === "basic") {
        authExample = `npm start -- --username your_username --password your_password --base-url ${baseUrl}`;
      } else {
        authExample = `npm start -- --token your_token_here --base-url ${baseUrl}`;
      }
    } else {
      authExample = `npm start -- --base-url ${baseUrl}`;
    }
...
- \`--base-url <url>\`: Base URL for API requests (defaults to ${baseUrl})
...
- \`src/utils/cli-args.js\`: CLI argument utilities
