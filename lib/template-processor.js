  ...
  handler: async (input) => {
    try {${
      ...
      if (input) {
        ...
        ${
          pathParams.length > 0
            ? pathParams
                .map(
                  (p) =>
                    `if (input.${p.name} !== undefined) pathParamsData.${p.name} = input.${p.name};`
                )
                .join("\n        ")
            : ""
        }
        ...
        ${
          queryParams.length > 0
            ? queryParams
                .map(
                  (p) =>
                    `if (input.${p.name} !== undefined) queryParamsData.${p.name} = input.${p.name};`
                )
                .join("\n        ")
            : ""
        }
        ...
        ${
          hasBody
            ? `if (input.body !== undefined) requestBodyData = input.body;`
            : ""
        }
      }
      ...
      ${pathParams
        .map(
          (p) =>
            `url = url.replace('{${p.name}}', encodeURIComponent(pathParamsData.${p.name} || ''));`
        )
        .join("\n      ")}`
          : ""
      }
      ...
      ${queryParams
        .map(
          (p) =>
            `if (queryParamsData.${p.name} !== undefined) queryParams.append('${p.name}', queryParamsData.${p.name});`
        )
        .join("\n      ")}
      ...
      return formatResponse({
        ...
        data: {
          ...
          response: data${
            hasParams
              ? `,
          parameters: { ...pathParamsData, ...queryParamsData }`
              : ""
          }${
      hasBody
        ? `,
