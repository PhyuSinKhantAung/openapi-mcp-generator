...
export class OpenApiMcpGenerator {
  ...
  async generateServer(specPath, outputDir, options = {}) {
    ...
    // Show correct authentication argument based on auth type
    if (authInfo.hasAuth) {
      if (authInfo.type === "bearer") {
        console.log(`  npm start -- --token your_token`);
        console.log(`\nFor MCP configuration:`);
        console.log(
          `  npx ${serverName
            .toLowerCase()
            .replace(/\s+/g, "-")} --token your_token`
        );
      } else if (authInfo.type === "apiKey") {
        console.log(`  npm start -- --api-key your_api_key`);
        console.log(`\nFor MCP configuration:`);
        console.log(
          `  npx ${serverName
            .toLowerCase()
            .replace(/\s+/g, "-")} --api-key your_api_key`
        );
      ...
      console.log(`\nFor MCP configuration:`);
      console.log(`  npx ${serverName.toLowerCase().replace(/\s+/g, "-")}`);
    }
  }
